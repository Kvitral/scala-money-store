...
class InMemoryAccountAlg[F[_] : Monad](accountState: Ref[F, Map[Long, Account]], logger: Logging[F]) extends AccountAlg[F] {

  override def getAccount(i: Long): F[Option[Account]] = for {
        ...
        accState.get(transaction.from)
          .fold((accState, Either.left[AccountServiceErrors, Unit](AccountNotFound))) { accFrom =>
            if (accFrom.balance < transaction.amount)
              ...
              accState.get(transaction.to)
                .fold(accState, Either.left[AccountServiceErrors, Unit](AccountNotFound)) { accTo =>
                  val res = accState
                    .updated(accFrom.id, accFrom.copy(balance = accFrom.balance - transaction.amount))
...
object InMemoryAccountAlg {
  def apply[F[_] : Monad](accountState: Ref[F, Map[Long, Account]], logger: Logging[F]): InMemoryAccountAlg[F] =
